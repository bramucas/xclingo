...
def add_to_base(rules_to_add, builder, t_option):
    """
...
def separate_labels_from_body(body_asts):
    """
...
def translate_to_fired_holds(rule_ast, control, builder, t_option):
    """
    ...
    if rule_ast.type == ast.ASTType.Rule:
        ...
        if rule_ast.is_show_all_rule() or rule_ast.is_trace_all_rule() or rule_ast.is_constraint():
            ...
            label_body, rest_body = separate_labels_from_body(rule_ast['body'])
        ...
        if rule_ast.type == ast.ASTType.Definition:
            add_to_base(str(rule_ast), builder, t_option)
